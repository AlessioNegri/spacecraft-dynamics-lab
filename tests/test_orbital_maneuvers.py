...
def test_hohmann_transfer_1():
    ...
    maneuver: om.ManeuverResult = om.OrbitalManeuvers.hohmann_transfer(attractor, rocket_motor, oe_1, oe_2, direction)
...
def test_hohmann_transfer_2():
    ...
    assert np.isclose(maneuver.dv[0].to_value(u.km / u.s) + maneuver.dv[1].to_value(u.km / u.s), 5.749, atol=1e-3)
    assert np.isclose(maneuver.dt[0].to_value(u.s), 4339.5, atol=1e-1)
    assert np.isclose((maneuver.dm[0].to_value(u.kg) + maneuver.dm[1].to_value(u.kg)) / 2000, 0.85, atol=1e-2)
...
def test_bi_elliptic_hohmann_transfer():
    ...
    assert np.isclose((maneuver.dv[0] + maneuver.dv[1] + maneuver.dv[2]).to_value(u.km / u.s), 4.0285, atol=1e-4)
    assert np.isclose((maneuver.dt[0] + maneuver.dt[1]).to_value(u.s), 488870, atol=1e0)
    ...
    assert np.isclose((hohmann.dv[0] + hohmann.dv[1]).to_value(u.km / u.s), 4.0463, atol=1e-4)
    assert np.isclose((hohmann.dt[0]).to_value(u.s), 65942, atol=1e0)
...
def test_phasing_maneuver_1():
    ...
    assert np.isclose((maneuver.dv[0] + maneuver.dv[1]).to_value(u.km / u.s), 0.4970, atol=1e-4)
...
def test_phasing_maneuver_2():
    ...
    assert np.isclose((maneuver.dv[0] + maneuver.dv[1]).to_value(u.km / u.s), 0.02252, atol=1e-5)
...
def test_plane_change_maneuver_from_dihedral_angle_1():
    ...
    assert np.isclose(maneuver.dv[0].to_value(u.km / u.s), 1.8191, atol=1e-4)
    ...
    assert np.isclose(maneuver.dv[0].to_value(u.km / u.s), 4.9239, atol=1e-4)
...
def test_plane_change_maneuver_from_dihedral_angle_2():
    ...
    assert np.isclose(maneuver.dv[0].to_value(u.km / u.s), 2.0165, atol=1e-4)
...
def test_non_impulsive_maneuver_1():
    ...
    rocket_motor.m_sc = maneuver.m_sc
...
def test_non_impulsive_maneuver_2():
    ...
    assert np.isclose(rocket_motor.m_sc.to_value(u.kg) - maneuver.m_sc.to_value(u.kg), 46.34, atol=1e-2)
